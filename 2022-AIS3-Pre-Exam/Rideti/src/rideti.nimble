...
task runr, "Runs rideti for current platform":
 exec &"nim c -r {releaseOpts} -o:rideti src/main.nim"

task rund, "Runs debug rideti for current platform":
 exec &"nim c -r {debugOpts} -o:rideti src/main.nim"

task release, "Builds rideti for current platform":
 exec &"nim c {releaseOpts} -o:rideti src/main.nim"

task webd, "Builds debug rideti for web":
 exec &"nim c {debugOpts} -d:emscripten -o:rideti.html src/main.nim"

task webr, "Builds release rideti for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:rideti.html src/main.nim"

task debug, "Builds debug rideti for current platform":
 exec &"nim c {debugOpts} -o:rideti_debug src/main.nim"
